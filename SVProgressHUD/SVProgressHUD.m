...
#endif /* if !defined(SV_APP_EXTENSIONS) */
...
+ (void)showWithText:(NSString *)status {
//    [self showImage:nil status:status];
    NSTimeInterval displayInterval = [self displayDurationForString:status];

    [[self sharedView] showStatus:status duration:displayInterval];
...
- (void)updateHUDFrame {
    ...
    BOOL progressUsed = self.imageView.isHidden;
...
- (void)updateHUDFrameWithText {
    // Check if an image or progress ring is displayed
    BOOL imageUsed = (self.imageView.image) && !(self.imageView.hidden) && (self.imageViewSize.height > 0 && self.imageViewSize.width > 0);
    BOOL progressUsed = NO;
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if (strongSelf) {
            ...
            if (self.graceTimeInterval > 0.0 && self.backgroundView.alpha == 0.0f) {
                strongSelf.graceTimer = [NSTimer timerWithTimeInterval:self.graceTimeInterval
                                                                target:strongSelf
                                                              selector:@selector(fadeIn:)
                                                              userInfo:nil
                                                               repeats:NO];
                [[NSRunLoop mainRunLoop] addTimer:strongSelf.graceTimer
...
- (void)showImage:(UIImage *)image status:(NSString *)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if (strongSelf) {
            ...
            strongSelf.imageView.hidden = image == nil;
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if (strongSelf) {
            ...
            if (self.graceTimeInterval > 0.0 && self.backgroundView.alpha == 0.0f) {
                ...
                [strongSelf fadeInWithText:@(duration)];
            }
        }
    }];
}

- (void)fadeInWithText:(id)data {
    // Update the HUDs frame to the new content and position HUD
    [self updateHUDFrameWithText];
    [self positionHUD:nil];
...
- (void)setRingNoTextRadius:(CGFloat)ringNoTextRadius {
    if (!_isInitializing) {
        _ringNoTextRadius = ringNoTextRadius;
    }
}
...
- (void)setShouldTintImages:(BOOL)shouldTintImages {
    if (!_isInitializing) {
        _shouldTintImages = shouldTintImages;
    }
}
...
- (void)setMaxSupportedWindowLevel:(UIWindowLevel)maxSupportedWindowLevel {
    if (!_isInitializing) {
        _maxSupportedWindowLevel = maxSupportedWindowLevel;
    }
}
